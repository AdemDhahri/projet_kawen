...
// Vérifiez si l'utilisateur est connecté en vérifiant s'il existe une session id_user
if (isset($_SESSION['id_user'])) {
    // Vous pouvez accéder aux données de l'utilisateur à partir de la session comme ceci :
    $id_user = $_SESSION['id_user'];
    $nom_comp = $_SESSION['nom_comp'];
    $adresse_mail = $_SESSION['adresse_mail'];
  
    $roles = $_SESSION['roles'];

    // Maintenant vous pouvez afficher ces données où vous le souhaitez dans la page
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['id_user']) && isset($_POST['nom_comp']) && isset($_POST['adresse_mail'])) {
        ...
        $stmt->bindParam(':nom_comp', $_POST['nom_comp']);
        $stmt->bindParam(':adresse_mail', $_POST['adresse_mail']);
        $stmt->bindParam(':id_user', $_POST['id_user']);
        // Exécution de la requête
        $stmt->execute();
        // Redirection vers la page d'origine ou affichage d'un message de confirmation
        echo "<script>window.location.href = 'compte_chercheur.php';</script>";
        exit();
    }
}
...
<div class="mt-3">
<h4> <?php echo $nom_comp; ?></h4>
<p class="text-secondary mb-1"><?php echo $roles; ?></p>
<p class="text-muted font-size-sm"><?php echo $adresse_mail; ?></p>
<button class="btn btn-primary"  type ="submit"><a href="exchange.php">changer en devise</a></button>
...
<div class="row">
<div class="col-sm-12">
<button class="btn btn-info" onclick="modifier('<?php echo htmlspecialchars($id_user);?>')">Modifier</button>
            
            <td>
                <!-- Formulaire pour modifier les données -->
                <form id="form_<?php echo $id_user ?>" style="display: none;" method="post">
                    <!-- Champs pour les données à modifier -->
                    <input type="hidden" name="id_user" value="<?php echo $id_user; ?>">
                    <input type="text" name="nom_comp" value="<?php echo $nom_comp; ?>">
                    <input type="text" name="adresse_mail" value="<?php echo $adresse_mail ?>">
...
<small>CHERCHEUR D'EMPLOI</small>
