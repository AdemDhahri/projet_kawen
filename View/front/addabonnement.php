...
include('../../Control/actions/AddAbonnementAction.php');

if(isset($_GET['idCours'])) {
    // Récupère l'ID du cours depuis l'URL
    $idCours = $_GET['idCours'];
} else {
    // Gérer le cas où l'ID du cours n'est pas passé dans l'URL
}
...
<script>
   document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('inscriptionFormAbonnement').addEventListener('submit', function(event) {
                ...
                if (MethodeDePaiement.toLowerCase() !== "cash" && MethodeDePaiement.toLowerCase() !== "espèces" && MethodeDePaiement.toLowerCase() !== "carte") {
                    alert("Veuillez entrer une méthode de paiement valide (lettres uniquement cash, espèces ou carte ).");
                    event.preventDefault(); // Empêche l'envoi du formulaire si la validation échoue
                }
...
<div class="container py-5">
    ...
    <form id="inscriptionFormAbonnement" method="post">
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="MethodeDePaiement" name="MethodeDePaiement" required>
        </div>
        <div class="mb-3">
            <label for="Prix" class="form-label">Prix:</label>
            <input type="number" class="form-control" id="Prix" name="Prix">
        </div>
        <input type="hidden" name="IdCoursA" value="<?= $idCours ?>">
        <div class="mb-3">
            <button type="submit" name="submit" class="btn btn-primary">Confirmer l'abonnement</button>
        </div>
    </form>
</div>
